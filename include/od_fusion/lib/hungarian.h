...
class HungarianOptimizer {
 ...
 public:
  ...
  void costs(std::vector<std::vector<T>>& costs) {  costs_ = costs; }
  ...
  // 成本矩阵 - 算法输入
  std::vector<std::vector<T>> costs_;

 private:
  enum  Mark { NONE=0, PRIME=1, STAR=2 };
  ...
  // 标记矩阵 - 记录 匹配/候选/NONE
  std::vector<std::vector<T>> marks_;
...
HungarianOptimizer<T>::HungarianOptimizer(const int max_optimization_size)
  ...
  costs_.reserve(max_optimization_size);
...
void HungarianOptimizer<T>::OptimizationInit() {
  ...
  for (size_t row = 0; row < matrix_size_; ++row) {
    for (size_t col = 0; col < matrix_size_; ++col) {
      marks_[row].push_back(Mark::NONE);
    }
  }
...
// ReduceRows → StarZeroes → CoverStarredZeroes → PrimeZeroes 
//     ↑                                      ↓
// AugmentPath ← MakeAugmentingPath ←───────┘
...
void HungarianOptimizer<T>::DoMunkres() {
  ...
  int num_iter = 0;
  fn_state_ = std::bind(&HungarianOptimizer::ReduceRows, this); // 状态机集中写法
...
// 目标是找到最大独立零元素集合：
// 1.逐行扫描：从第0行到最后一行
// 2.跳过已覆盖行列：避免在同一行或列放置多个星标
// 3.标记第一个零元素：在未覆盖的行列中找到第一个零，标记为星标
// 4.覆盖相应行列：确保该行该列不会再有其他星标
// 5.继续下一行：使用 break 跳到下一行
template <typename T>
...
void HungarianOptimizer<T>::CoverStarredZeroes() {
  ...
  // 统计包含星标的列数量
  for (size_t col = 0; col < matrix_size_; ++col) {
    if (ColContainsStar(col)) { // 根据星标，更新 覆盖的列
      CoverCol(col);
      num_covered++;
    }
  }
...
void HungarianOptimizer<T>::PrimeZeroes() {
  ...
  for (;;) {
    ...
    if (!FindZero(&zero_row, &zero_col)) { // 
      // No uncovered zeroes. //0全部被覆盖cover，需产生新的0
      fn_state_ = std::bind(&HungarianOptimizer::AugmentPath, this);
      return;
    }

    Prime(zero_row, zero_col); // NONE -> PRIME
    int star_col = FindStarInRow(zero_row); // 当前行找STAR的列
    ...
    if (star_col != kHungarianOptimizerColNotFound) {
      ...
      assignments_[0] = first_assignment; // 未匹配点 行列序号 对
      // 寻找增广路径
...
void HungarianOptimizer<T>::MakeAugmentingPath() {
  ...
  // rows_covered_，cols_covered_ 更新
  ClearCovers();
  // marks_更新： PRIME -> NONE
  ClearPrimes();
  fn_state_ = std::bind(&HungarianOptimizer::CoverStarredZeroes, this);
